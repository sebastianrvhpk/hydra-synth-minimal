...
Video synth engine for [hydra](https://github.com/ojack/hydra), now running on a raw WebGPU renderer.

This branch is WebGPU-first. There is no WebGL/regl fallback path.
...
### WebGPU requirements
- Browser with WebGPU enabled (recent Chrome/Edge or equivalent)
- Secure context (`https://` or `http://localhost`)
- Hardware/driver support for a compatible GPU adapter

If WebGPU is unavailable, Hydra throws an explicit initialization error.
