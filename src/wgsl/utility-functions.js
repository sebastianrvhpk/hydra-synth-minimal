...
const selectUtility = (utilityName, selected) => {
  ...
  dependencies.forEach((dependencyName) => selectUtility(dependencyName, selected))
  selected.add(utilityName)
}

export const collectUtilityDeclarations = (wgslFunctions = []) => {
  const functionBodies = wgslFunctions
    .map((transform) => transform.transform.wgsl)
    .join('\n')

  const selectedUtilities = new Set()
  UTILITY_ORDER.forEach((utilityName) => {
  ...
  return UTILITY_ORDER
    ...
    .map((utilityName) => utilityWgsl[utilityName].wgsl)
    .join('\n')
}

export default utilityWgsl
