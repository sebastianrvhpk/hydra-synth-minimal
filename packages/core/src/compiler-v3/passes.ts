...
import type { HydraDependencyEdgeV3, HydraKernelGraphV3, HydraKernelNodeV3 } from '../ir-v3/types.js'
import type {
...
interface ResourceIntervalV3 {
  start: number
  end: number
}

interface VariantPassCandidateV3 extends HydraExecutionVariantCandidateV3 {
  pass: HydraCompiledPass
}

interface PlannerSlotStateV3 {
  id: string
  aliasGroup: string
  end: number
  bytes: number
...
export interface HydraResourcePlanningResultV3 {
  allocations: HydraResourceAllocationPlanV3[]
  peakTransientBytes: number
  totalPlannedBytes: number
}

const resourceByteEstimate = (resource: HydraKernelGraphV3['resources'][number]): number => {
...
const variantOfPass = (pass: HydraCompiledPass): HydraExecutionStepV3['variant'] => {
...
const evaluateCandidateLegality = (
  pass: HydraCompiledPass,
  capabilityProfile: HydraPlannerCapabilityProfileV3
...
const selectVariantCandidate = (
  candidates: VariantPassCandidateV3[],
  policy: HydraExecutionPlanDiagnosticsV3['selectedVariantPolicy']
): VariantPassCandidateV3 => {
  const preferenceOrder: Record<
  HydraExecutionPlanDiagnosticsV3['selectedVariantPolicy'],
  Array<HydraExecutionStepV3['variant']>
  > = {
...
const computeResourceIntervals = (
  ...
  orderedNodes: HydraKernelNodeV3[]
): Map<string, ResourceIntervalV3> => {
  const resourceIds = new Set(graph.resources.map((resource) => resource.id))
  const intervals = new Map<string, ResourceIntervalV3>()
...
export const inferAndOrderNodesV3 = (graph: HydraKernelGraphV3): HydraKernelNodeV3[] => {
  ...
  if (ordered.length === graph.nodes.length) {
    ...
    return ordered.map((id) => byId.get(id)).filter((entry): entry is HydraKernelNodeV3 => Boolean(entry))
  }

  return graph.nodes.slice()
}

export const buildExecutionBarriersV3 = (
...
export const planResourceAllocationsV3 = (
  graph: HydraKernelGraphV3,
  orderedNodes: HydraKernelNodeV3[]
): HydraResourcePlanningResultV3 => {
  const intervals = computeResourceIntervals(graph, orderedNodes)
  const slotState = new Map<string, PlannerSlotStateV3>()
  const slotsByAlias = new Map<string, PlannerSlotStateV3[]>()
  ...
  const allocations: HydraResourceAllocationPlanV3[] = resources.map((resource) => {
    ...
    if (aliasable) {
      ...
      if (selected) {
        ...
        const created: PlannerSlotStateV3 = {
          id: slotId,
          aliasGroup,
          end: interval.end,
          bytes: plannedBytes
...
export const buildExecutionStepsV3 = (
  orderedNodes: HydraKernelNodeV3[],
  compiledPassByNodeId: Map<string, HydraCompiledPass>,
  barriers: HydraExecutionBarrierV3[],
  selectedVariantPolicy: HydraExecutionPlanDiagnosticsV3['selectedVariantPolicy'],
  capabilityProfile: HydraPlannerCapabilityProfileV3,
  primitiveByNodeId: Map<string, HydraExecutionPrimitiveSelectionV3> = new Map()
): HydraExecutionStepV3[] => {
  const barriersByNode = new Map<string, HydraExecutionBarrierV3[]>()
  ...
  const steps: HydraExecutionStepV3[] = []
...
export const scoreExecutionPlanV3 = (
  steps: HydraExecutionStepV3[],
  _allocations: HydraResourceAllocationPlanV3[],
  selectedVariantPolicy: HydraExecutionPlanDiagnosticsV3['selectedVariantPolicy'],
  peakTransientBytes: number,
  totalPlannedBytes: number,
  barrierCount: number
