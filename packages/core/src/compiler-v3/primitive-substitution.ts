import {
  getPrimitiveDescriptorByKindV3
} from '../primitives-v3/descriptors.js'
import { PRIMITIVE_WGSL_MODULES_V3 } from '../primitives-v3/wgsl/index.js'
import type { HydraKernelNodeV3 } from '../ir-v3/types.js'
import type { HydraCompiledPass } from '../types.js'
import type { HydraExecutionPrimitiveSelectionV3 } from './types.js'
...
const inferPrimitiveKindForNode = (node: HydraKernelNodeV3): HydraExecutionPrimitiveSelectionV3['kind'] | null => {
...
const createPrimitiveSelection = (
  kind: HydraExecutionPrimitiveSelectionV3['kind'],
  substituted: boolean,
  note?: string
): HydraExecutionPrimitiveSelectionV3 | null => {
  const descriptor = getPrimitiveDescriptorByKindV3(kind)
  if (!descriptor) return null
  const module = PRIMITIVE_WGSL_MODULES_V3[descriptor.wgslModuleId]
...
export const applyPrimitiveSubstitutionsV3 = (
  orderedNodes: HydraKernelNodeV3[],
  compiledPassByNodeId: Map<string, HydraCompiledPass>,
  maxDynamicUniforms: number
): Map<string, HydraExecutionPrimitiveSelectionV3> => {
  const primitiveByNodeId = new Map<string, HydraExecutionPrimitiveSelectionV3>()
