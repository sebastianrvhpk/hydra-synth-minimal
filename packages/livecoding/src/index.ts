...
const defaultEvaluate = (code: string, scope: Record<string, unknown>): unknown => {
  const evaluator = new Function('scope', `with (scope) { ${code} }`)
...
export const attachLivecoding = (
  ...
  const run = (code: string): unknown => {
    ...
    syncFromEngine()
    const result = evaluate(code, targetGlobal)
    syncFromGlobal()
    return result
  }
