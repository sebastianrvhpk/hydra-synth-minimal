// Curated sketches used by the canonical Hydra web editor.
// Source: https://github.com/hydra-synth/hydra/blob/1fab4f65d5797d3f9cf2c83286cbad1d51cdd47a/src/stores/examples.json

export const hydraExamplesSource = Object.freeze({
...
export const hydraExamples = Object.freeze([
  {
    "sketch_id": "example_0",
...
const licenseLine = '// licensed with CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/'

const clampRandomIndex = (random, length) => {
  const value = Number(random())
  const normalized = Number.isFinite(value) ? Math.max(0, Math.min(0.9999999999999999, value)) : 0
  return Math.floor(normalized * length)
}

export const decodeHydraExampleCode = (encodedCode) =>
  decodeURIComponent(atob(String(encodedCode ?? '').trim()))

export const getHydraExampleName = (example, code = decodeHydraExampleCode(example?.code)) => {
  const comment = String(code)
    .split(/\r?\n/u)
    .map((line) => line.match(/^\s*\/\/\s*(.+?)\s*$/u)?.[1] ?? '')
    .find(Boolean)
  return comment || String(example?.sketch_id ?? 'Hydra sketch')
}

export const materializeHydraExample = (example) => {
  if (!example) return null
  const decodedCode = decodeHydraExampleCode(example.code).trim()
  const code = decodedCode.includes(licenseLine)
    ? decodedCode
    : `${licenseLine}\n\n${decodedCode}`
  return {
    id: String(example.sketch_id),
    name: getHydraExampleName(example, decodedCode),
    code,
    source: hydraExamplesSource
  }
}

export const createHydraExamplePicker = ({
  examples = hydraExamples,
  random = Math.random
} = {}) => {
  let previousIndex = -1
  ...
  return () => {
    ...
    let index = clampRandomIndex(random, length)
    if (length > 1 && index === previousIndex) {
      index = (index + 1 + clampRandomIndex(random, length - 1)) % length
    }
    previousIndex = index
    return materializeHydraExample(examples[index])
  }
}
