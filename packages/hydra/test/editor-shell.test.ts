...
describe('live editor shell', () => {
  ...
  it('backs livecode indicators with balanced contrast surfaces', () => {
    ...
    expect(indexHtml).toMatch(/\.cm-typed-flash\s*\{[^}]*background:\s*rgba\(255, 204, 102, 0\.12\);[^}]*color:\s*inherit;[^}]*font-weight:\s*inherit;/u)
    expect(indexHtml).not.toMatch(/\.cm-typed-flash\s*\{[^}]*color:\s*#050505;/u)
    expect(indexHtml).toContain('}, 240)')
  })
})
