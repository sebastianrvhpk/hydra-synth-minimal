...
describe('live editor shell', () => {
  ...
  it('backs livecode indicators with balanced contrast surfaces', () => {
    ...
    expect(indexHtml).toMatch(/\.cm-focused \.cm-selectionBackground\s*\{[^}]*background:\s*rgba\(255, 107, 214, 0\.28\);[^}]*rgba\(255, 255, 255, 0\.96\)[^}]*rgba\(3, 6, 8, 0\.98\)/u)
    ...
    expect(indexHtml).toMatch(/const codeMaterialChromeSelector = \[[\s\S]*'\.cm-typed-flash',[\s\S]*'\.cm-parameter-scope',[\s\S]*'\.cm-call-scope'/u)
