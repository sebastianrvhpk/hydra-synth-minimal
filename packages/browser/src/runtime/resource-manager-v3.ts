...
export interface HydraResourceResidencySnapshotV3 {
...
export class HydraResourceManagerV3 {
  ...
  getOrCreateStorageBuffer (slot: string, requiredBytes: number): GPUBuffer {
    ...
    const created = this.renderer.createStorageBuffer(`hydra-v3-buffer-${slot}`, minBytes)
  ...
  getOrCreateIndirectArgsBuffer (slot: string, offset = 0): GPUBuffer {
    ...
    if (existing) existing.buffer.destroy()
    const created = this.renderer.createStorageBuffer(`hydra-v3-indirect-${slot}`, bytes)
  ...
  getOrCreateQueueCounterBuffer (slot: string): GPUBuffer {
    ...
    if (existing) existing.buffer.destroy()
    const created = this.renderer.createStorageBuffer(`hydra-v3-queue-counter-${slot}`, bytes)
  ...
  getResidencySnapshot (): HydraResourceResidencySnapshotV3 {
