...
describe('DSL compatibility matrix', () => {
  ...
  it('supports graph-valued arguments in scalar generator parameter slots', () => {
    const { registry, output } = createRegistry()
    const upstreamOutput = { id: 0, getTexture: () => null }

    registry.generators
      .noise(
        registry.generators
          .osc(3, 0.04, 0)
          .blend(registry.generators.src(upstreamOutput), 0.25)
          .mult(3)
          .add(1),
        registry.generators
          .shape(4, 0.2, 0.01)
